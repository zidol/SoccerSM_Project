	...
	public void insertBuy(List<CartDataBean> lists,
		...
		try {
			...
			for(int i=0; i<lists.size(); i++) {
				...
				sql = "insert into buy (buy_id,buyer,product_id,product_title,buy_price,buy_count,"
						+ "product_image,buy_date,account,deliveryName,deliveryTel,deliveryAddress)"
						+ " values(?,?,?,?,?,?,?,?,?,?,?,?)";
				...
				pstmt = conn.prepareStatement("select product_count from product where product_id=?");
